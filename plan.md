...
├── reloopy.sock                    ← Unix Domain Socket
...
```json
{
  "capabilities": {
    "filesystem": {
      "read": ["~/.reloopy/state/", "~/.reloopy/constitution/"],
      "write": ["~/.reloopy/state/memory.json", "~/.reloopy/state/context.json"]
    },
    "network": {
      "allowed": false,
      "whitelist": []
