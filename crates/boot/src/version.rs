...
impl VersionManager {
    ...
    pub fn commit_version_source(
        ...
        //    Use `-f` so untracked files (e.g. Cargo.lock generated by a
        //    previous build) do not block the checkout.
        let co = Command::new("git")
            .args(["checkout", "-f", &version_info.version])
            .current_dir(src)
            .output()
    ...
    pub fn switch_to(&mut self, version: &str) -> Result<String, String> {
        ...
        // Use `-f` so untracked files (e.g. Cargo.lock from a build) do not
        // block the checkout.
        let o = Command::new("git")
            .args(["checkout", "-f", version])
            .current_dir(&self.source_dir)
            .output()
    ...
    pub fn rollback(&mut self) -> Result<String, String> {
        ...
        let o = Command::new("git")
            .args(["checkout", "-f", &rollback_version])
            .current_dir(&self.source_dir)
            .output()
