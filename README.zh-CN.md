...
`start.sh` 会以 release 模式构建整个工作区，然后按正确顺序依次拉起 **Boot → Compiler → Peripheral**。看到 ➜ 提示后打开 <http://127.0.0.1:7700> 即可。

> 还没有 API Key？直接运行 `./start.sh`——Boot 和 Compiler 仍会正常启动，便于探索架构。
